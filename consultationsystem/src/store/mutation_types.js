//****全局用户mutation
export const GET_TOKEN = 'GET_TOKEN' //获取用户token
export const SET_USERINFO = 'SET_USERINFO' //获取用户info
export const UPDATE_USERROLE = 'UPDATE_USERROLE' //修改用户角色
export const UPDATE_USERINFO = 'UPDATE_USERINFO' //修改用户info
export const UPLOAD_USER_PROGRESS = 'UPLOAD_USER_PROGRESS' //修改用户进度条
export const UPDATE_PAGE_PERMISSIONS = 'UPDATE_PAGE_PERMISSIONS'//初始化页面权限
export const RESET_PAGE_PERMISSIONS = 'RESET_PAGE_PERMISSIONS'//重置页面权限
//****全局面包屑mutation
export const UPDATE_BREAD = 'UPDATE_BREAD' //修改面包屑
//****全局侧边栏mutation
export const ASIDE_CLICK = 'ASIDE_CLICK' //侧边栏选中
export const ITEM_CLICK = 'ITEM_CLICK' //更新侧边栏选中序号
export const MAIN_CLICK = 'MAIN_CLICK' //主界面选中
export const UPDATE_ASIDE_MENU = 'UPDATE_ASIDE_MENU'//初始化侧边栏
export const RESET_ASIDE_MENU = 'RESET_ASIDE_MENU'//重置侧边栏